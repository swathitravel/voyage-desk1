...
let state=structuredClone(blank),user=null,unsubscribe=null,ready=false,saveTimer=null,modalType=null,editingTripId=null,editingRecordId=null,rate=null;
...
function openModal(type,id=null){
  if(!activeTrip()){toast("Create a trip first");switchView("trips");return}
  modalType=type;
  editingRecordId=id;
  const s=schemas[type];
  const existing=id ? state.records[s.target].find(x=>x.id===id) : null;
  $("#modalTitle").textContent=existing ? `Edit ${s.title.replace(/^Add /,"").toLowerCase()}` : s.title;
  $("#modalFields").innerHTML=s.fields.map(([n,l,t,o])=>`<label class="${t==="textarea"?"wide":""}">${l}${t==="select"?`<select name="${n}">${o.map(q=>`<option>${q}</option>`).join("")}</select>`:t==="timezone"?`<select name="${n}">${tzOptions}</select>`:t==="textarea"?`<textarea name="${n}"></textarea>`:`<input name="${n}" type="${t}" ${t==="number"?'step="0.01" min="0"':""}>`}</label>`).join("");
  if(existing){
    const form=$("#modalForm");
    s.fields.forEach(([name])=>{
...
$("#modalForm").addEventListener("submit",e=>{
  e.preventDefault();
  const s=schemas[modalType];
  const values=Object.fromEntries(new FormData(e.currentTarget).entries());
  ["cost","estimated","actual","quantity"].forEach(k=>{if(k in values)values[k]=Number(values[k]||0)});
  if(editingRecordId){
    const existing=state.records[s.target].find(x=>x.id===editingRecordId);
    if(existing) Object.assign(existing,values);
  }else{
    values.id=uid();
    values.tripId=state.activeTripId;
    if(["tasks","packing"].includes(s.target)) values.done=false;
    state.records[s.target].push(values);
  }
...
window.removeRecord=removeRecord;window.toggleRecord=toggleRecord;window.editRecord=(type,id)=>openModal(type,id);
...
$("#modal").addEventListener("close",()=>{if($("#modal").returnValue==="cancel")editingRecordId=null});
